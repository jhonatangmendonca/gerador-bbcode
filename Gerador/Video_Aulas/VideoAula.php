...
<body>
    ...
    <main class="container" style="margin-top: 80px;margin-bottom: 80px;">
        <form class="center-align" action=" https://cliente.####.club/enviar-videoaula.php" method="post" target="_blank">
            <textarea id="descrCopia" style="margin-top: -1000px;">
                    <?php
                    $idInserido =  $_GET["idInserido"];

                    $query = " SELECT T.*, V.*, A.* FROM videoaulas V 
                    inner join assinaturas A On V.FkUsuario = A.FkUsuario 
                    inner join tags T On T.FkUsuario = A.FkUsuario 
                    WHERE V.Pk = $idInserido";
                    $resultado = $CONEXAO->query($query);
                    if ($resultado) {
                        if ($resultado->num_rows > 0) {
                            while ($linha = $resultado->fetch_assoc()) {
                ...
                if ($resultado) {
                    if ($resultado->num_rows > 0) {
                        while ($linha = $resultado->fetch_assoc()) {
                            ...
                            <div class="col-12 <?php echo $alinhamentoTexto ?> mb-5">
                                <b>Autor: </b><?php echo $linha["Autor"] ?><br />
                                ...
                                <b>Data de Lançamento: </b><?php echo $linha["Data"] ?>
                            </div>
