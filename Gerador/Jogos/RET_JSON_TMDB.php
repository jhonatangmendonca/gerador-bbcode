...
#	       			 PAGINA DE DEBUG	                #
#########################################################
if (!empty($_POST["imdb"] or $_GET["imdb"])) {
	...
	//IMDB - OMDB
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, "https://store.steampowered.com/api/appdetails/?appids=730&key=&l=brazilian");
	...
	if ($imdb['Type'] == "movie") {
		...
		$result_count_movie = count($result['id']);
