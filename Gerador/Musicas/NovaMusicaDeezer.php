...
if (empty($_GET['idDeezer'])) {
    header("Location: NovaMusica.php?idDeezer=noData");
} else {
    $idDeezer = $_GET['idDeezer'];
}
$curl = curl_init();
curl_setopt_array($curl, array(
    CURLOPT_URL => "https://deezerdevs-deezer.p.rapidapi.com/album/" . $idDeezer,
    ...
    CURLOPT_HTTPHEADER => array(
        ...
        "x-rapidapi-key: 5852f0f81emsh145f2af328b1ce9p1a63dcjsn6b9075bb491b"
    ),
));
...
curl_close($curl);
$deezer = json_decode($response, true);
if ($err) {
    echo "cURL Error #:" . $err;
} else if (isset($deezer['error'])) {
    header("Location: NovaMusica.php?idDeezer=noData");
} else {
    function dateFormat($deezer, $months, $monthsPtBr)
    {
        $str = $deezer['release_date'];
...
<body>
    <ul id="slide-out" class="side-nav fixed z-depth-2">
        ...
        <ul class="collapsible" data-collapsible="accordion">
            <?php if ($idUsuario == 1) { ?>
                <li id="dash_brands">
                    <div id="dash_brands_header" class="collapsible-header"><b>Administração</b>
                        <i style="float: right;" class="material-icons">keyboard_arrow_down</i>
                    </div>
                    <div id="dash_brands_body" class="collapsible-body">
                        <ul>
                            <li id="brands_brand">
                                <a style="text-decoration: none;" href="./../Admin/Index.php">Usuários</a>
                            </li>
                        </ul>
                    </div>
                </li>
        ...
        <form action="gera.php" method="post" class="p-3">
            ...
            <div class="row">
                <div class="input-field col s12 m6 l6 xl6">
                    <input id="artista" value="<?php echo $deezer['artist']['name']; ?>" name="artista" type="text" class="validate" required="">
                    <label class="active" for="artista">Artista</label>
                </div>

                <div class="input-field col s12 m6 l6 xl6">
                    <input id="album" value="<?php echo $deezer['title']; ?>" name="album" type="text" class="validate" required="">
            ...
            <div class="row">
                <div class="input-field col s12 m6 l6 xl6">
                    <input id="data" value="<?php dateFormat($deezer, $months, $monthsPtBr); ?>" name="data" type="text" class="validate" required="">
                    <label class="active" for="data">Data de Lançamento</label>
                </div>

                <div class="input-field col s12 m6 l6 xl6">
                    <input id="capa" value="<?php echo $deezer['cover_big']; ?>" name="capa" type="text" class="validate" required="">
            ...
            <div class="row">
                <div class="input-field col s12 m6 l6 xl6">
                    <input id="loja" value="<?php echo $deezer['link']; ?>" name="loja" type="text" class="validate" required="">
            ...
            <div class="row">
                <div class="input-field col s12">
                    <textarea id="faixas" name="faixas" class="align-center materialize-textarea validate" required=""><?php
                                                                                                                        if ($deezer['tracks']['data'][0]['artist']['name'] !== $deezer['artist']['name']) {
                                                                                                                            for ($i = 0; $i < count($deezer['tracks']['data']); $i++) {
                                                                                                                                echo ($i + 1) . '. ' . $deezer['tracks']['data'][$i]['title_short'] . ' - ' . $deezer['tracks']['data'][$i]['artist']['name'] . "\n";
                                                                                                                            }
                                                                                                                        } else {
                                                                                                                            for ($i = 0; $i < count($deezer['tracks']['data']); $i++) {
                                                                                                                                echo ($i + 1) . '. ' . $deezer['tracks']['data'][$i]['title_short'] . "\n";
                                                                                                                            }
                                                                                                                        }
                                                                                                                        ?>
                   </textarea>
